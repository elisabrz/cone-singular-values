"""
Test Schur vs Schur - Following paper's exact approach
...
- Uses accuracy=1e-10 (stricter than Schur vs Nonneg)
...
def test_schur_vs_schur(n, timelimit=10, max_iter=500, accuracy=1e-10, 
                         verbose=True, test_incremental=True):
    """
    Test Schur vs Schur following paper's approach
    ...
    accuracy : float
        Convergence tolerance (1e-10 for Schur vs Schur, stricter than Schur vs Nonneg!)
    verbose : bool
        Print progress
    ...
    if verbose:
        print(f"\n{'='*70}")
        print(f"SCHUR vs SCHUR (n={n})")
        print(f"{'='*70}")
    
    # Load reference
    ref = get_reference_data()
    problem_name = f'Schur_vs_Schur_n{n}'
    ...
    if verbose:
        ...
        print(f"  Accuracy: {accuracy} (stricter for Schur vs Schur)")
    ...
    while T < timelimit:
        ...
        options_eao = {
            ...
            'G': G_schur,
            'H': G_schur,  # Same as G for Schur vs Schur
            'maxiter': max_iter,
            'accuracy': accuracy,
            'beta': 0.5,
            'display': 0,
    ...
    if verbose:
        ...
        print(f"E-AO:        {results['eao']['angle']:.8f}π  time={results['eao']['time']:.3f}s  restarts={results['eao']['num_restarts']}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        result = test_schur_vs_schur(n, timelimit=timelimit, verbose=True)
    else:
        ...
        all_results = []
        for n in dimensions:
            result = test_schur_vs_schur(n, timelimit=10, verbose=True)
